...
void set_dtc(ControllerStruct *controller){
	...
	if(!controller->phase_order){
		__HAL_TIM_SET_COMPARE(&TIM_PWM, TIM_CHANNEL_1, ((TIM_PWM.Instance->ARR))*dtc_u);
		__HAL_TIM_SET_COMPARE(&TIM_PWM, TIM_CHANNEL_2, ((TIM_PWM.Instance->ARR))*dtc_v);
		__HAL_TIM_SET_COMPARE(&TIM_PWM, TIM_CHANNEL_3, ((TIM_PWM.Instance->ARR))*dtc_w);
	}
	else{
		__HAL_TIM_SET_COMPARE(&TIM_PWM, TIM_CHANNEL_1, ((TIM_PWM.Instance->ARR))*dtc_u);
		__HAL_TIM_SET_COMPARE(&TIM_PWM, TIM_CHANNEL_2, ((TIM_PWM.Instance->ARR))*dtc_v);
...
//void set_dtc2(float dtc_a, float dtc_b, float dtc_c, float amplitude){
...
//	if(!reverse){ 
//		
//		__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, ((htim1.Instance->ARR))*dtc_u*amplitude);
//		__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, ((htim1.Instance->ARR))*dtc_v*amplitude);
//		__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_3, ((htim1.Instance->ARR))*dtc_w*amplitude);
//	}
//	else{					// dao thu tu pha
//		__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, ((htim1.Instance->ARR))*dtc_u*amplitude);
//		__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, ((htim1.Instance->ARR))*dtc_v*amplitude);
//		__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_3, ((htim1.Instance->ARR))*dtc_w*amplitude);
//	}
//}
...
void setPhaseVoltage(float Uq, float Ud, float angle_el, ControllerStruct *controller) {
  ...
  int sector;
  float _ca,_sa;
//	if(!sm_or_svm){		// sm_or_svm = 0, sinusoidal modulation
...
void svm(float v_max, float u, float v, float w, float *dtc_u, float *dtc_v, float *dtc_w){
    ...
    float v_midpoint = .5f*(DTC_MAX+DTC_MIN);											// duty cycle of output PWM for 1/2 of max_voltage
...
void commutate(ControllerStruct *controller, float elec_angle, float elec_velocity, float velocity, float mech_angle)
{
	/* Do Field Oriented Control */
		...
		controller->theta_elec = elec_angle;
		controller->dtheta_elec = elec_velocity;
		controller->dtheta_mech = velocity/GR;
		controller->theta_mech = mech_angle/GR;
